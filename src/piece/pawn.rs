        ...
        match piece {
                    ...
                    2 => {
                        if let (Some(square1), Some(square2)) = (
                            ...
                            if square1.available() && square2.available() {
                                let forward_square =
                                    Square::new(Some(piece.clone()), Coordinate::new(column, 4)); // 2 square mouv
                                available_moves.push(forward_square);
                            }
                        }
                    }
                        ...
                        if let Some((previous_square, current_square)) = self.previous_move {
                            match previous_square.get_piece() {
                                Some(Piece::Pawn(Color::Black)) if previous_square.get_line().unwrap() == 7 && 
                                    ...
                                    => {
                                    // En passant ifmove
                                    let en_passant_square = Square::new(Some(piece.clone()), Coordinate::new(column-1, line+1)); // 1 square mouv
            ...
            Piece::Pawn(Color::Black) => {
                ...
                match line {
                    7 => {
                        if let (Some(square1), Some(square2)) = (
                            self.get_square(Coordinate::new(column, 6)),
                            self.get_square(Coordinate::new(column, 5)),
                        ) {
                            if square1.available() && square2.available() {
                                let forward_square =
                                    Square::new(Some(piece.clone()), Coordinate::new(column, 5)); // 2 square mouv
                                available_moves.push(forward_square);
                            }
                        }
                    }
